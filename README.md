...
- **LLaMA 3 70B via Groq Cloud (free API, no local GPU needed))
...
ChromaDB (cosine similarity, HNSW)
  ...
  Query (JD) ──embed──▶ cosine search ──▶ Ranked candidates
                                                │
                                                ▼
                                  LLaMA 3 70B (Groq Cloud) AI Summary
                                  • Strengths / Gaps
                                  • Fit score 0-100
...
```bash
# Add your GROQ_API_KEY to the .env file (free at https://console.groq.com)
